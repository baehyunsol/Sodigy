...
pub fn lt_bi(
    ...
    if lhs_neg != rhs_neg {
        ...
        match cmp_ubi(lhs, rhs) {
            ...
            // -3 > -4
            std::cmp::Ordering::Less if lhs_neg => false,
            // 3 < 4
            std::cmp::Ordering::Less => true,
            // -4 < -3
            std::cmp::Ordering::Greater if lhs_neg => true,
            // 4 > 3
...
pub fn gt_bi(
    ...
    if lhs_neg != rhs_neg {
        ...
        match cmp_ubi(lhs, rhs) {
            std::cmp::Ordering::Equal => false,
            // -3 > -4
            std::cmp::Ordering::Less if lhs_neg => true,
            // 3 < 4
            std::cmp::Ordering::Less => false,
            // -4 < -3
            std::cmp::Ordering::Greater if lhs_neg => false,
            // 4 > 3
...
pub fn cmp_ubi(lhs: &[u32], rhs: &[u32]) -> std::cmp::Ordering {
    if lhs.len() > rhs.len() {
    ...
    match (lhs.len(), rhs.len()) {
        ...
        (l, r) if l == r => {
            ...
            match lhs_hi.cmp(&rhs_hi) {
                std::cmp::Ordering::Less => vec![0],
                std::cmp::Ordering::Equal => match cmp_ubi(lhs, rhs) {
                    std::cmp::Ordering::Greater | std::cmp::Ordering::Equal => vec![1],
                    std::cmp::Ordering::Less => vec![0],
                },
                std::cmp::Ordering::Greater => {
