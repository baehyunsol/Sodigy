...
pub enum Field {
    ...
    // From here are special fields that are generated by post-mir.
    EnumDiscriminant,

    // Enum payloads are tuples. `PatternKind::Struct` and `PatternKind::TupleStruct` are
    // lowered to tuples in the end.
    EnumPayload {
        // In order to execute the code, you only need `payload`. You don't have to
        // know `variant`. But in order to solve the type, you have to know `variant`.
        variant: usize,
        payload: usize,
    },
