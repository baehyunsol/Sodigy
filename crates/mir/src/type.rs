    ...
    let t = match r#type {
        Type::Data { constructor_def_span, args, .. } => {
            if *constructor_def_span == global_context.get_lang_item_span_id("type.Tuple") {
                ...
                if let Some(struct_shape) = global_context.struct_shapes.unwrap().get(&def_span) {
                    match &field[0] {
                        Field::Index(i) if *i >= 0 => global_context.get_type(&struct_shape.fields[*i as usize].name_span).unwrap(),
                        Field::Name { name, .. } => {
                            let mut result: Option<Type> = None;

                            for field in struct_shape.fields.iter() {
                                if field.name == *name {
                                    result = global_context.get_type(&field.name_span);
                                    break;
                                }
                            }
...
// Monomorphization-related functions from here.
// These functions are supposed to be in inter-mir, but `get_def_span_from_id` is used
// by `type_of_field`, which is in mir, so these functions are in mir.
