...
`String` is an alias for `[Char]` and `Bytes` is an alias for `[Byte]`. But when do we apply these aliases?

Also, can users define their own type aliases?

1. `type String = [Char];`
2. `type MyMap<T> = Map<T, Int>` is okay -> lhs can have generic args.
3. Type aliases are applied before the type inference.
...
// 와 골때리는 cycle 떠오름 -> 이것도 잡을 수 있나??
// 1. bar에 있는 default value를 expand하고 나면 `let` statement만 보고도 cycle을 잡을 수 있음.
//    expand하는 timing이 한참 나중이란게 문제...
// 2. 이건 걍 runtime에 죽어도 상관없는 거 아님? 그냥 함수 선언하고 infinite recursion 때려버리면 죽잖아. 그거랑 똑같은 거지...
let foo = bar();
fn bar(x=foo) = x;
```
...
나중에는 prelude에 대응되는 코드들을 다 실제로 작성해서 (in Sodigy), 컴파일러가 집어넣으면 됨!
