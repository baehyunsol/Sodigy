...
pub fn run(options: CompilerOption, prev_output: Option<CompilerOutput>) -> CompilerOutput {
    ...
    for (is_necessary, is_file, path) in [
        ...
        // for now, `__sdg_cache` directories are generated by `generate_path_for_ir` func
        // (options.save_ir, false, `__sdg_cache` for all directories),
...
Options:
    ...
    --save-ir [true|false]          Save intermediate representations (default: true)
                                    The compiler makes `__sdg_cache` directory, and save the intermediate
