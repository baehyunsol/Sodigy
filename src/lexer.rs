...
fn lex_token(s: &[u8], mut ind: usize, session: &mut LocalParseSession) -> Result<(Token, usize), ParseError> {
    ...
    match s[ind] {
        ...
        | b'!' | b'=' | b'<' | b'>'
        | b',' | b'.' | b':' | b';'
        | b'&' | b'|' => lex_op_tokens(s, ind, session),
...
fn lex_op_tokens(s: &[u8], ind: usize, session: &mut LocalParseSession) -> Result<(Token, usize), ParseError> {
    let curr_span = Span::new(session.curr_file, ind);
...
For consecutive range operators (which is likely a semantic error), try `(1..)..`".to_string()
                ))
            }
...
fn skip_whitespaces_and_comments(s: &[u8], mut ind: usize, session: &mut LocalParseSession) -> Result<usize, ParseError> {
    let curr_span = Span::new(session.curr_file, ind);
